...
//                                            .data = {0, 0 , 0 , 0 ,0 ,0 ,0 ,0}};
static twai_message_t data_message = {.identifier = ID_MASTER_DATA,
                                            .data_length_code = 8,
...
void app_main(void)
{
	...
	esp_err_t res = 0xff;
    while (1) {
		res = twai_transmit(&data_message, portMAX_DELAY);
		ESP_LOGI(EXAMPLE_TAG, "Transmitted data (id:0x%x, res:0x%x)", data_message.identifier, res);
		data_message.identifier += 1;
		vTaskDelay(pdMS_TO_TICKS(DATA_PERIOD_MS));
	}
